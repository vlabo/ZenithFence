...
// callouts reach the device through `get_device()`, and the host fuzz harness
// installs a mock `Device` here before driving the callouts. These two items
// use no kernel/windows-sys types.
...
// Everything below is the real WDM/WFP driver entry plumbing. It depends on
// windows-sys and the real `wdk` IRP helpers, so it is excluded from the mock
// (host) build, where there is no kernel to register with.
#[cfg(not(feature = "mock"))]
mod kernel_entry {
    use super::{get_device, DEVICE};
