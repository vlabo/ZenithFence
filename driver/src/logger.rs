    ...
    pub fn flush(&mut self) -> Vec<Box<LogLine>> {
        ...
        if let Some(lines) = &mut self.log_lines {
            ...
            for i in start_index..start_index + count {
                let index = i % lines.len();
                let ptr = lines[index].swap(core::ptr::null_mut(), Ordering::Acquire);
                unsafe {
                    if !ptr.is_null() {
